...
async function main(): Promise<void> {
  ...
  if (args.length === 0) {
    ...
    console.error('Commands: init, whoami, status, peers, announce, send, decode, serve, diagnose');
    process.exit(1);
  }
  ...
  try {
    switch (command) {
      ...
      case 'serve':
        ...
        break;
      default:
        console.error(`Error: Unknown command '${command}'. Use: init, whoami, status, peers, announce, send, decode, serve, diagnose`);
        process.exit(1);
    }
  } catch (e) {
